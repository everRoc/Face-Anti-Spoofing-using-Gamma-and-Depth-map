...
def gammaTrans(img):
    ...
    if random.random()>0.5:
        gamma = 1 + random.random()
    else:
        gamma = 1/(1.0+random.random())
    img0 = np.power(im0/255.0,gamma)
    img1 = np.power(im1/255.0,gamma)
    img2 = np.power(im2/255.0,gamma)
    img[:,:,0] = img0*255
    img[:,:,1] = img1*255
    img[:,:,2] = img2*255
    #print("before:",im0[5,5],"gamma:",gamma,"after:",img[5,5,0])
    return img
...
class Spoofing_train(Dataset):
    ...
    def __init__(self, root_dir,  transform=None):
        ...
        #'''
        if root_dir.split("oulu")[1] == "/dev_face1/":
            
            self.root_depth = root_dir.split("oulu")[0] + "oulu/dev_depth/"

        elif root_dir.split("oulu")[1] == "/train_face1/":
            
            self.root_depth = root_dir.split("oulu")[0] + "oulu/train_depth/"
        #print(self.root_depth)
        #'''
        #self.root_depth = "/home/chang/dataset/B_depth/"
...
'''
rootDir = "/home/chang/dataset/oulu/dev_face1/"
